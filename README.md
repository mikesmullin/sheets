...
A spreadsheet where **rows are entities, columns are pipeline stages, and Play runs any
lasso-selected slice of (entity × stage) through a concurrency-controlled queue** — with an
Angela chat sidebar that authors the stage scripts from one-sentence prompts.
...
```sh
bun install       # agl-ai + angela come from bun link (see below)
bun link          # register `sheets` on $PATH (~/.bun/bin)
cd examples/fruit
sheets serve      # http://localhost:4400
# in another terminal:
sheets run --range C:C -c 8    # deterministic demo column
```
...
```
sheets serve|init|run|stop|status|concurrency|ls|cat|clean|help
sheets run --range B:B -c 5          # play a column, follow the gradient meter
sheets run --entities apple..fig --stages fruit-color --dry   # dry-run + diff, no persist
```

The server is required: CLI and browser discover it via `.sheets/server.json` and exit with
advice if it's not running.
...
## Known v1 divergences from PLAN1

- Cell views are **static HTML via x-html** (m-js has no `x-mount` in the published build);
  interactive in-cell widgets are v2.
- Focus mode uses a plain textarea editor (CodeMirror slated for v2).
- Comparator (`sort`) columns load docs in memory — fine to ~50k rows; SQL-path sorts scale.
- `sheets angela` TUI chat, paste-pad, bulk row add: deferred (plan M7).
